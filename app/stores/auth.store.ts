...
import { useStorage } from "~/utils/helpers/storage.helper";

export const useAuthStore = defineStore("auth", () => {
  const tokens = ref<AuthTokens | null>(useStorage().tokens);
  const user = ref<User | null>(useStorage().user);
  const loading = ref<boolean>(false);

  const clearAuth = () => {
    tokens.value = null;
    user.value = null;
    useStorage().clearAuth();
  };
